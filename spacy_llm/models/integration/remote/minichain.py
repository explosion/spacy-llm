...
def _check_installation() -> None:
    ...
    if not has_minichain:
        raise ValueError(
            "The MiniChain model requires `minichain` to be installed, which it is not. See "
...
def query_minichain() -> (
    ...
    def prompt(
        model: "minichain.backend.Backend", prompts: Iterable[str]
    ) -> Iterable[str]:
        @minichain.prompt(model)
        def _prompt(_model: "minichain.base.Prompt.Model", prompt_text: str) -> str:
            return _model(prompt_text)
...
@registry.llm_models("spacy.MiniChain.v1")
def model_minichain(
    api: str,
    query: Optional[
        Callable[["minichain.backend.Backend", Iterable[str]], Iterable[str]]
