...
def test_entity_linking_config(cfg_string, request):
...
def test_entity_linking_predict(cfg_string, request):
...
def test_el_io(cfg_string, request):
    cfg = request.getfixturevalue(cfg_string)
    orig_config = Config().from_str(cfg)
    nlp = spacy.util.load_model_from_config(orig_config, auto_fill=True)
    assert nlp.pipe_names == ["llm"]
...
def test_el_init(noop_config, n_prompt_examples: int):
