...
from spacy_llm.tasks import make_spancat_task
...
def test_spancat_zero_shot_task(text, response, gold_spans):
    labels = "PER,ORG,LOC"
    llm_spancat = make_spancat_task(labels=labels)
    # Prepare doc
    nlp = spacy.blank("xx")
...
def test_spancat_labels(response, normalizer, gold_spans):
    ...
    labels = "PER,ORG,LOC"
    llm_spancat = make_spancat_task(labels=labels, normalizer=normalizer)
    # Prepare doc
    nlp = spacy.blank("xx")
    doc_in = nlp.make_doc(text)
    # Pass to the parser
...
def test_spancat_alignment(response, alignment_mode, gold_spans):
    text = "Jean Jacques and Jaime went to the library."
    labels = "PER,ORG,LOC"
    llm_spancat = make_spancat_task(labels=labels, alignment_mode=alignment_mode)
    # Prepare doc
    nlp = spacy.blank("xx")
    doc_in = nlp.make_doc(text)
    # Pass to the parser
...
def test_invalid_alignment_mode():
    ...
    with pytest.raises(ValueError, match="Unsupported alignment mode 'invalid"):
        make_spancat_task(labels=labels, alignment_mode="invalid")
...
def test_jinja_template_rendering_without_examples():
    ...
    llm_spancat = make_spancat_task(labels=labels, examples=None)
...
def test_jinja_template_rendering_with_examples(examples_path):
    ...
    llm_spancat = make_spancat_task(labels=labels, examples=examples)
    prompt = list(llm_spancat.generate_prompts([doc]))[0]
...
def test_example_not_following_basemodel():
    ...
    with make_tempdir() as tmpdir:
        ...
        with pytest.raises(ValidationError):
            make_spancat_task(labels="PER,ORG,LOC", examples=fewshot_reader(tmp_path))
