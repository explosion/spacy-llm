...
- A [_Task_](api#Tasks), defining the prompt to send to the LLM as well as the functionality to parse the resulting response
  back into structured fields on spaCy's [Doc](https://spacy.io/api/doc) objects.
- A [_Backend_](api#Backends) defining the model to use and how to connect to it. Note that `spacy-llm` supports both access to external
...
For more background information, see the [DollyHF](api#spacydollyhfv1) section.
...
For more background information, see the [OpenAI](api#OpenAI) section.
