...
Of course, there can be components in your system for which the power of an LLM is fully justified. If you want a system that can synthesise information from multiple documents in subtle ways and generate a nuanced summary for you, bigger is better. That's a difficult task with no single right answer, so a small model probably won't be able to get you good performance.
